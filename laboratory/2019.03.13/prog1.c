...
void *work_routine(void *arg) {
  ...
  while (not_done) {
    ...
    printf("\nwork_routine()\tthread %d\tmy count %d, total_count %d\n", *my_id,
           thread_counts[*my_id], total_count);
...
extern int main(int argc, char **argv) {
  int t, i, *id_arg, prio_min, prio_max;
  ...
  if (argc != 2) {
    printf(
        "Argument required: "
        "d=default,r=roundrobin,t=roundrobin/priority,f=fifo,i=inherit,p=fifo/"
        "priority\n");
    exit(1);
  }
  ...
  for (i = 0; i < NUM_THREADS; i++) {
    ...
    if (sched == DEFAULT)
      pthread_create(&(threads[i]), NULL, work_routine, (void *)&(id_arg[i]));
    else {
      /*
       * Just for the fun of it, let's up our priority, and
       * run with policy FIFO
       */
      priority_param.sched_priority = PRI_FIFO_MAX;
      ...
      if (sched == INHERIT) {
        ...
        priority_param.sched_priority =
            prio_min + (i) * (prio_max - prio_min) / (NUM_THREADS - 1);
        printf("main()\t\t\t\tsetting priority(%d-%d) of thread %d to %d\n",
               prio_min, prio_max, i, priority_param.sched_priority);
        if (pthread_attr_setschedparam(&pthread_custom_attr, &priority_param) !=
            0)
          fprintf(stderr, "pthread_attr_setschedparam failed\n");
      }
    }
    pthread_create(&(threads[i]), &pthread_custom_attr, work_routine,
                   (void *)&(id_arg[i]));
  }
  ...
  for (i = 0; i < NUM_THREADS; i++) {
    if (total_count != 0) {
      printf("main()\t\t\t\tthread %d, count %d percent %f\n", i,
             thread_counts[i], (float)thread_counts[i] / (float)total_count);
    }
  }
